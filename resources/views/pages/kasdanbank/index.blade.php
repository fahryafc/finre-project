            ...
            <div class="card-wrapper">
                <div class="card h-full flex flex-col bg-green-100">
                    <div class="p-6 flex-grow flex flex-col justify-between relative">
                        ...
                        <p class="text-green-700 font-bold text-3xl truncate mt-auto pt-4">
                            {{ 'Rp. ' . number_format($chart['uang_masuk'], 0, ',', '.') }}</p>
                    </div>
                </div>
            </div>
            <!-- Card Uang Keluar -->
            <div class="card-wrapper">
                <div class="card h-full flex flex-col bg-red-100">
                    <div class="p-6 flex-grow flex flex-col justify-between relative">
                        ...
                        <p class="text-red-700 font-bold text-3xl truncate mt-auto pt-4">
                            {{ 'Rp. ' . number_format($chart['uang_keluar'], 0, ',', '.') }}</p>
                    </div>
                </div>
            </div>
        </div>
    ...
    <div class="card mt-10 p-5">
        <div class="card-header mb-5">
            <div class="flex justify-between items-center">
                ...
                <button id="tambahKasBank" class="btn bg-primary text-white" data-fc-target="modalKasBank"
                    data-fc-type="modal" type="button"><i class="mgc_add_fill text-base me-4"></i>
                    Tambah Kas & Bank
                </button>
            </div>
        </div>
                            ...
                            <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                                <thead class="bg-gray-50 dark:bg-gray-700">
                                    <tr>
                                        ...
                                        <th scope="col"
                                            class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">Kas &
                                            Bank</th>
                                        <th scope="col"
                                            class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">Kode
                                        </th>
                                        <th scope="col"
                                            class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">Saldo
                                            Awal</th>
                                        <th scope="col"
                                            class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">Uang
                                            Masuk</th>
                                        <th scope="col"
                                            class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">Uang
                                            Keluar</th>
                                    ...
                                    @foreach ($kas_bank as $key)
                                        <tr>
                                            <td
                                                ...
                                                {{ $counter + ($kas_bank->currentPage() - 1) * $kas_bank->perPage() }}</td>
                                            <td
                                                class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-800 dark:text-gray-200">
                                                {{ $key->nama_akun }}</td>
                                            <td
                                            ...
                                            <td
                                                ...
                                                {{ 'Rp. ' . number_format($key->saldo_awal, 0, '.', '.') }}</td>
                                            <td
                                                class="px-6 py-4 whitespace-nowrap text-sm text-gray-800 dark:text-gray-200">
                                                {{ 'Rp. ' . number_format($key->debit ?? 0, 0, '.', '.') }}</td>
                                            <td
                                                class="px-6 py-4 whitespace-nowrap text-sm text-gray-800 dark:text-gray-200">
                                                {{ 'Rp. ' . number_format($key->kredit ?? 0, 0, '.', '.') }}</td>
                                            ...
                                            <td
                                                class="px-6 py-4 whitespace-nowrap text-sm text-gray-800 dark:text-gray-200">
                                                {{ 'Rp. ' . number_format($saldo_akhir ?? 0, 0, '.', '.') }}</td>
                                            @csrf
                                            @method('DELETE')
                                            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                                <form action="{{ route('kasdanbank.destroy', $key->id_akun) }}"
                                                    method="POST"
                                                ...
                                                <button type="button" data-id="{{ $key->id_akun }}"
                                                    class="edit btn rounded-full bg-warning/25 text-warning hover:bg-warning hover:text-white"
                                                    data-fc-target="modalKasBank" data-fc-type="modal"
            ...
            <form action="{{ route('kasdanbank.store') }}" method="POST" id="formKasBank">
                ...
                <div class="px-4 py-4 overflow-y-auto max-h-[60vh]">
                    <div class="grid gap-4">
                        ...
                        <div>
                            <div class="mb-3">
                                <label for="nama_akun" class="text-gray-800 text-sm font-medium inline-block mb-2">Nama
                                    Akun</label>
                                <input type="text" class="form-input" id="nama_akun" name="nama_akun"
                                    aria-describedby="nama_akun" placeholder="Masukan Nama Akun" required>
                            </div>
                            <div class="mb-3">
                                <label for="kode_akun" class="text-gray-800 text-sm font-medium inline-block mb-2">Kode
                                    Akun</label>
                                <input type="text" class="form-input" id="kode_akun" name="kode_akun"
                                    aria-describedby="kode_akun" placeholder="Masukan Kode Akun" required>
                            </div>
                            <div class="mb-3">
                                <label for="kategori_akun"
                                    class="text-gray-800 text-sm font-medium inline-block mb-2">Kategori Akun</label>
                                <select id="kategori_akun"
                                    class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
                                    name="kategori_akun" required>
                            ...
                            <div class="mb-3">
                                <label for="subakun" class="text-gray-800 text-sm font-medium inline-block mb-2">Sub
                                    Kategori Akun</label>
                                <select id="subakun"
                                    class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
                                    name="subakun">
