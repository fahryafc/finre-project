        ...
        <div class="card p-5">
            <div class="card-header">
                <div class="flex justify-between items-center">
                    ...
                    <button id="tambahAkun" class="btn bg-primary text-white" data-fc-target="modalTambahAkun"
                ...
                <table id="akun-table">
                    <thead>
                        <tr>
                            ...
                            <th scope="col" class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">Kode
                            </th>
                            <th scope="col" class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">Nama
                                Akun</th>
                            <th scope="col" class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">
                                Kategori</th>
                            <th scope="col" class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">Saldo
                            </th>
                            <th scope="col" class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">Aksi
                            </th>
                        </tr>
                    </thead>
                    <tbody>
                        ...
                        @foreach ($akun as $key)
                            <tr id="akun-{{ $key->id_akun }}">
                                ...
                                @else
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-800 dark:text-gray-200">Rp. 0
                                    </td>
                                @endif
                                @csrf
                                @method('DELETE')
                                <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                    @if ($key->saldo < 1)
                                        <a href="{{ route('akun.destroy', $key->id_akun) }}" data-confirm-delete="true"
                                            class="btn rounded-full bg-danger/25 text-danger hover:bg-danger hover:text-white"><i
                                                class="mgc_delete_2_line"></i></a>
                                    @endif
                                    <button type="button" data-id="{{ $key->id_akun }}" data-fc-target="modalTambahAkun"
                                        data-fc-type="modal"
                                        class="edit btn rounded-full bg-warning/25 text-warning hover:bg-warning hover:text-white"><i
                        ...
                        <div>
                            <div class="mb-3">
                                ...
                                <input type="text" class="form-input" id="nama_akun" name="nama_akun"
                                    aria-describedby="nama_akun" placeholder="Masukan Nama Akun">
                            </div>
                            <div class="mb-3">
                                ...
                                <input type="text" class="form-input" id="kode_akun" name="kode_akun"
                                    aria-describedby="kode_akun" placeholder="Masukan Kode Akun">
                            </div>
                            <div class="mb-3">
    ...
    <script>
        ...
        $(document).ready(function() {
            ...
            $.ajax({
                ...
                success: function(response) {
                    ...
                    response.forEach(function(kategori) {
                        kategoriSelect.append('<option value="' + kategori.id_kategori_akun +
            ...
            $('#kategori_akun').on('change', async function() {
                ...
                // Kosongkan dropdown dan tampilkan teks loading
                subakunSelect.innerHTML = '<option value="">Memuat...</option>';
                subakunSelect.disabled = true;
                ...
                if (kategoriId) {
                    try {
                        ...
                        subakunSelect.innerHTML =
                            '<option value="">-- Pilih Sub Kategori Akun --</option>';
                        result.forEach(sub => {
                            const option = document.createElement('option');
                            option.value = sub.subakun; // Sesuaikan dengan field JSON
                            option.textContent = sub.subakun;
                            subakunSelect.appendChild(option);
                        });
                    } catch (error) {
                        console.error("Error fetching subkategori:", error);
                        subakunSelect.innerHTML = '<option value="">Gagal memuat data</option>';
                    } finally {
                        subakunSelect.disabled = false;
                    }
                } else {
            ...
            $(document).on('click', '.edit', async function() {
                ...
                resultKategoriAkun.forEach(kategori => {
                    if (kategori.id_kategori_akun == data.id_kategori_akun) {
                        ...
                        kategoriSelect.append('<option value="' + kategori.nama_kategori +
