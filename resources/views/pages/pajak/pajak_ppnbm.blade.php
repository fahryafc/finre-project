                        ...
                        @foreach ($pajak_ppnbm as $ppnbm)
                            <tr>
                                <td
                                    ...
                                    {{ $counter + ($pajak_ppnbm->currentPage() - 1) * $pajak_ppnbm->perPage() }}</td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-800 dark:text-gray-200">
                                    {{ $ppnbm->deskripsi_barang }}</td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-800 dark:text-gray-200">
                                    {{ 'Rp. ' . number_format($ppnbm->harga_barang, 0, '.', '.') }}</td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-800 dark:text-gray-200">
                                    {{ $ppnbm->tarif_ppnbm }}%</td>
                                @if ($ppnbm->jenis_pajak == 'Pajak Keluaran')
                                    <td>
                                        <span
                                            class="whitespace-nowrap gap-1.5 py-1.5 px-3 rounded-full text-xs font-medium bg-green-500 text-white">{{ $ppnbm->jenis_pajak }}</span>
                                    </td>
                                @else
                                    <td>
                                        <span
                                            class="inline-flex items-center gap-1.5 py-1.5 px-3 rounded-full text-xs font-medium bg-red-500 text-white">{{ $ppnbm->jenis_pajak }}</span>
                                    </td>
                                @endif
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-800 dark:text-gray-200">
                                    {{ 'Rp. ' . number_format($ppnbm->ppnbm_dikenakan, 0, '.', '.') }}</td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-800 dark:text-gray-200">
                                    {{ \Carbon\Carbon::parse($ppnbm->tgl_transaksi)->format('d-m-Y') }}</td>
                            </tr>
                            @php $counter++; @endphp
                        @endforeach
                    </tbody>
