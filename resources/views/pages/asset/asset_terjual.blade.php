                    ...
                    <thead>
                        <tr>
                            ...
                            <th scope="col" class="px-6 py-3 text-end text-xs font-medium text-gray-500 uppercase">
                                Depresiasi</th>
                            <th scope="col" class="px-6 py-3 text-end text-xs font-medium text-gray-500 uppercase">Total
                                Nilai Aset</th>
                            <th scope="col" class="px-6 py-3 text-end text-xs font-medium text-gray-500 uppercase">Total
                    ...
                    <tbody>
                        ...
                        @foreach ($assetTerjual as $terjual)
                            <tr>
                                ...
                                <td
                                    class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-800 dark:text-gray-200">
                                    {{ 'Rp. ' . number_format($terjual->harga_pelepasan, 0, '.', '.') }}
                                </td>
                                <!-- Harga Buku -->
                                <td
                                    class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-800 dark:text-gray-200">
                                ...
                                @php
                                    // Ambil nilai asli
                                    $nominal = str_replace('.', '', $terjual->nominal_keuntungan_kerugian); // Hilangkan titik
                                    $angka = (int) filter_var($nominal, FILTER_SANITIZE_NUMBER_INT); // Ambil angka

                                    // Tentukan warna dan tanda
                                    $warna = str_starts_with($nominal, '+')
                                        ? 'bg-green-100 text-green-700'
                                        : 'bg-red-100 text-red-700';
                                @endphp

                                <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                    <span class="px-2 py-1 rounded-full font-semibold {{ $warna }}">
                                        {{ str_starts_with($nominal, '+') ? '+' : '-' }}
