...
class PengeluaranController extends Controller
{
    ...
    public function destroy(string $id_pengeluaran)
    {
        ...
        try {
            ...
            // Kembalikan saldo hutang/piutang jika ada
            if ($pengeluaran->hutang) {
                $hutangPiutang = DB::table('hutangpiutang')
                    ->where('id_kontak', $pengeluaran->id_kontak)
                    ->where('kategori', $pengeluaran->kategori)
                    ->where('tgl_jatuh_tempo', $pengeluaran->tgl_jatuh_tempo)
                    ->where('jenis', 'hutang')
                    ->first();

                if ($hutangPiutang) {
                    $newSaldo = $hutangPiutang->nominal - $pengeluaran->nominal_hutang;

                    if ($newSaldo <= 0) {
                        // Jika saldo hutang habis, hapus record
                        DB::table('hutangpiutang')->where('id_hutangpiutang', $hutangPiutang->id_hutangpiutang)->delete();
                    } else {
                        // Jika masih ada saldo, perbarui nominal
                        DB::table('hutangpiutang')->where('id_hutangpiutang', $hutangPiutang->id_hutangpiutang)
                            ->update(['nominal' => $newSaldo]);
                    }
                }
            }

            // Hapus data pajak jika ada
            if ($pengeluaran->pajak) {
                if ($pengeluaran->jns_pajak == 'ppn') {
                    DB::table('pajak_ppn')->where('kode_reff', $pengeluaran->kode_reff_pajak)->delete();
                } elseif ($pengeluaran->jns_pajak == 'ppnbm') {
                    DB::table('pajak_ppnbm')->where('kode_reff', $pengeluaran->kode_reff_pajak)->delete();
                } elseif ($pengeluaran->jns_pajak == 'pph') {
                    DB::table('pajak_pph')->where('kode_reff', $pengeluaran->kode_reff_pajak)->delete();
                }
            }

            // Kembalikan saldo kas & bank jika ada
            $arusUang = Arusuang::where('kode_akun', $pengeluaran->akun_pembayaran)
                ->where('nominal', $pengeluaran->biaya)
                ->where('type', 'uang keluar')
                ->first();

            if ($arusUang) {
                $kasBank = Akun::where('type', 'Kas & Bank')->where('kode_akun', $pengeluaran->akun_pembayaran)->first();
                if ($kasBank) {
                    // Tambahkan kembali saldo ke akun kas & bank
                    $kasBank->update(['saldo' => $kasBank->saldo + $pengeluaran->biaya]);
                }

                // Hapus record dari arus uang
                $arusUang->delete();
            }
